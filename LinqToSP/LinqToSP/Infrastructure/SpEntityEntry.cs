      ...
      where TContext : class, ISpEntryDataContext
  {
    ...
    public void Update()
    {
      lock (_lock)
      {
        ...
        if (!hasChanges && typeof(ISpChangeTracker).IsAssignableFrom(Entity.GetType()))
        {
          var currentValues = (Entity as ISpChangeTracker).DetectChanges(
             OriginalValues.ToDictionary(val => val.Key, val => val.Value),
             CurrentValues.ToDictionary(val => val.Key, val => val.Value));
          if (currentValues != null)
          {
            hasChanges = currentValues.Count > 0;
            CurrentValues.Clear();
