        ...
        where TContext : class, ISpEntryDataContext
    {
        ...
        private bool DetectChanges(MemberInfo member, FieldAttribute field, object originalValue, ref object currentValue)
        {
            ...
            else
            {
                ...
                else if (field.Name == "ContentTypeId")
                {
                    if (!Equals(originalValue, currentValue))
                    {
                        string originalContentTypeId = Convert.ToString(originalValue) ?? "";
                        string currentContentTypeId = Convert.ToString(currentValue) ?? "";

                        if (!originalContentTypeId.StartsWith(currentContentTypeId))
                        {
                            isChanged = true;
                            currentValue = currentContentTypeId;
                        }
                    }
                    else
                    {
                ...
                else if (typeof(ISpChangeTracker).IsAssignableFrom(Entity.GetType()))
                {
