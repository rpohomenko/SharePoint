...
namespace SP.Client.Linq.Provisioning
{
      ...
      where TEntity : class, IListItemEntity, new()
  {
    ...
    {
      if (contentType.Sealed || contentType.ReadOnly || !fields.Any()) return;
      ...
      if (ProvisionHandlers != null)
      {
        ...
        if (_contentTypes.Count > 0)
        {
          foreach (var contentType in _contentTypes)
          {
            ...
            {
              string ctName;
              string ctId = contentType.Value.Id.StringValue;
              ...
              if (listContentType == null)
              {
                listContentType = _list.ContentTypes.AddExistingContentType(contentType.Value);
                _list.Context.Load(listContentType);
                _list.Context.ExecuteQuery();

                SetFieldLinks(contentType.Value, _fields.Values, comparison);
              }
            }
          }
        }

        if (_list != null && listHandler.List.Behavior != ProvisionBehavior.None)
        {
          foreach (var field in _fields)
          {
            if (field.Key.Field.Level == ProvisionLevel.Web)
            {
              Field existField = _list.Fields.GetByInternalNameOrTitle(field.Key.Field.Name);
              ...
              if (existField == null)
              {
                if (forceOverwrite || field.Key.Field.Behavior == ProvisionBehavior.Overwrite)
                {
                  if (typeof(CalculatedFieldAttribute).IsAssignableFrom(field.Key.Field.GetType()))
                  {
                    string fieldRefs = (field.Key.Field as CalculatedFieldAttribute).FieldRefs == null
                        ? null
                        : string.Join("", (field.Key.Field as CalculatedFieldAttribute).FieldRefs.Select(fieldRef => new Caml.CamlFieldRef() { Name = fieldRef }.ToString()));
                    string formula = (field.Key.Field as CalculatedFieldAttribute).Formula;

                    if (!string.IsNullOrEmpty(fieldRefs) && !string.IsNullOrEmpty(formula))
                    {
                      var refFields = new List<Field>();
                      foreach (string fieldName in (field.Key.Field as CalculatedFieldAttribute).FieldRefs)
                      {
                      ...
                      (field.Key.Field as CalculatedFieldAttribute).Formula = formula;

                      field.Value.ReplaceFormula(formula, (field.Key.Field as CalculatedFieldAttribute).FieldRefs);
                    }
                  }
                }

                Field newField = _list.Fields.Add(field.Value);
                if (forceOverwrite || field.Key.Field.Behavior == ProvisionBehavior.Overwrite)
                {
                  newField = field.Key.ApplyField(newField);
                  newField.Update();
                }
