...
export default class AddFormFieldPanel extends React.Component<IAddFormFieldPanelProps, IAddFormFieldPanelState> {
  ...
  private loadContentTypeFields(list: IList, contentTypeId: string): Promise<IFormField[]> {
    return new Promise<IFormField[]>((resolve: (options: IFormField[]) => void, reject: (error: any) => void) => {
      try {
        return list.contentTypes.getById(contentTypeId).fields.filter('Hidden eq false')/*.orderBy('Title')*/.select('Id', 'InternalName', 'EntityPropertyName', 'Title', 'Description', 'FieldTypeKind', 'AllowMultipleValues', 'Required', 'ReadOnlyField', 'RichText', 'DisplayFormat', 'LookupField', 'LookupList', 'LookupWebId', 'IsRelationship', 'PrimaryFieldId', 'Choices', 'DefaultValue'/*, 'SchemaXml'*/).get()
          .then(fields => {
  ...
  private get_Field(field: IFieldInfo, fields: IFieldInfo[]): IFormField {
    let formField = { Id: field.Id, Name: field.EntityPropertyName || field.InternalName, Title: field.Title, Description: field.Description, DataType: SPService.get_DataType(field), Required: field.Required, ReadOnly: field.ReadOnlyField, DefaultValue: field.DefaultValue } as IFormField;
